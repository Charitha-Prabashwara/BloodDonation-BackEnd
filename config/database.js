...
mongoose.connection.on('connected', async function() {
    ...
    await writeLog('Connection established to MongoDB',errorCode);

});

mongoose.connection.on('reconnected', async function() {
    //console.log( getCurrentDateTime() +'Reconnecting to MongoDB');
    await writeLog('Reconnecting to MongoDB',errorCode);
});
...
process.on('SIGINT', function() {
    mongoose.connection.close( async function () {
        ...
        await writeLog('Force to close the MongoDB connection',errorCode);
        process.exit(0);
    });
});
